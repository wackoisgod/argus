    ...
    fn begin_vsync_thread(&self) {
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                // TaskManager patch: adaptive invalidation cadence. Full rate
                // (~66Hz cap) while the user interacted within the last 2s;
                // ~10Hz once idle. Input instantly restores full rate, so
                // interaction latency is unaffected while idle wakeups drop
                // ~6x. Minimized windows are skipped entirely.
                let mut last_redraw = std::time::Instant::now();
                loop {
                    let idle_ms = millis_since_start()
                        .saturating_sub(LAST_INPUT_MS.load(std::sync::atomic::Ordering::Relaxed));
                    let relaxed = idle_ms >= 2000;
                    if relaxed {
                        // Idle: no animation to pace — sleep instead of
                        // waking at monitor refresh rate. First input after
                        // idle pays ≤100ms before full rate resumes.
                    ...
                    if !relaxed && last_redraw.elapsed() < std::time::Duration::from_millis(15) {
                        continue;
                    }
                    last_redraw = std::time::Instant::now();
                    for hwnd in all_windows.read().iter() {
                        unsafe {
                            // TaskManager patch: minimized windows can't show
                            // anything — don't wake the UI thread for them.
                            if ::windows::Win32::UI::WindowsAndMessaging::IsIconic(hwnd.as_raw())
                                .as_bool()
                            {
                                continue;
                            }
