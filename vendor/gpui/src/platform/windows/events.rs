...
// TaskManager patch: adaptive vsync cadence. The wndproc stamps this on user
// input; the vsync thread reads it to decide between full and relaxed
// invalidation rates.
pub(crate) static LAST_INPUT_MS: std::sync::atomic::AtomicU64 =
...
#[inline]
fn note_user_input() {
    LAST_INPUT_MS.store(millis_since_start(), std::sync::atomic::Ordering::Relaxed);
}
...
impl WindowsWindowInner {
    pub(crate) fn handle_msg(
        ...
        // TaskManager patch: user interaction (mouse, keyboard, focus,
        // move/resize) restores the full vsync invalidation rate.
        match msg {
            ...
            | WM_MOVE
            | WM_ENTERSIZEMOVE
            | WM_ENTERMENULOOP => note_user_input(),
            _ => {}
        }
